    ...
    private var toolbar: some View {
        HStack(spacing: 10) {
            HStack(spacing: 6) {
                ...
                TextField("Search tasks & bugs (name or #id)…", text: $search)
            ...
            Picker("Scope", selection: $store.scopeAll) {
                Text("Current sprint").tag(false); Text("All").tag(true)
            }.pickerStyle(.segmented).labelsHidden().frame(width: 200)
            .onChange(of: store.scopeAll) { _ in Task { await store.refresh() } }
